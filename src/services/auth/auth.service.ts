...
import { User, VerificationStatus, authRepository, verificationRepository } from '../../repositories';
...
export async function verifyUser(request: VerifyUserRequest): Promise<ResponseEntity> {
	...
	const verificationStatus: string | CustomError = await verificationRepository.verifyAuthCode(request.email);
